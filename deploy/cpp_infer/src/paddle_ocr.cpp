    ...
    const char* char_list_file, const char* cls_model_dir)
{
    ...
    if (det_model_dir) {
        ocr_ptr->det_ptr = std::make_unique<PaddleOCR::DBDetector>(
            det_model_dir,
            FLAGS_use_gpu, FLAGS_gpu_id, FLAGS_gpu_mem,
    ...
    if (rec_model_dir && char_list_file) {
        ocr_ptr->rec_ptr = std::make_unique<PaddleOCR::CRNNRecognizer>(
            rec_model_dir,
            FLAGS_use_gpu, FLAGS_gpu_id, FLAGS_gpu_mem,
            FLAGS_cpu_threads, FLAGS_enable_mkldnn, char_list_file,
    ...
    double* out_times, size_t* out_times_size)
{
    ...
    *out_size = boxes.size();
    ...
    for (const auto& box : boxes) {
        ...
        //}
        crop_list.emplace_back(std::move(crop));
    }
    std::vector<string> strs_res(crop_list.size());
    std::vector<float> scores(crop_list.size());
    ocr_ptr->rec_ptr->Run(crop_list, strs_res, scores, all_times);
